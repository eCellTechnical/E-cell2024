...
const EventRegistrationPopup = ({
    ...
    try {
      if (activeTab === "create") {
        ...
        if (resp.data.success) {
          setSuccess(true);
          setTimeout(() => (window.location.href = "/endeavour/profile?events"), 2000);
        } else {
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentTransactionId.trim().length !== 12) throw new Error("Transaction ID must be exactly 12 characters long");
  ...
  const displayEventName =
    eventName || (formData.eventSlug ? formData.eventSlug.replace(/-/g, " ") : "the event");
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
        <div className="bg-gradient-to-b from-[#1e2a38] to-[#18222D] rounded-xl w-full max-w-3xl overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto border border-gray-700">
          <div className="flex justify-between items-center p-5 border-b border-gray-700 bg-[#1c2634]">
            ...
            <button onClick={onClose} className="text-gray-400 hover:text-white transition-colors hover:bg-gray-700 rounded-full p-1">
              <X size={24} />
            </button>
          </div>
           ...
           {displayEventName!="Entertainment Eve"&& <div className="flex mb-8 bg-[#111920] rounded-lg p-1.5 max-w-sm mx-auto">
            ...
            {!success ? (
              <form onSubmit={handleContinue} className="space-y-6 md:space-y-8">
                ...
                <div className="pt-4">
                  <button type="submit" disabled={isSubmitting} className="w-full py-3 rounded-md text-black bg-[#00fcb8] hover:bg-[#00d06d] disabled:opacity-50">
                    {isSubmitting ? 
                      (activeTab === "create" ? "Continuing..." : "Joining...") : 
                      (activeTab === "create" ? "Continue" : "Join Team")}
                  </button>
                </div>
              </form>
            ) : (
              <div className="p-6 bg-[#0d3331] border border-[#00fcb8] rounded-md text-[#00fcb8]">
                <p>{activeTab === "create" ? "Team created! Proceed to payment." : "Successfully joined the team!"}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-gradient-to-b from-[#1e2a38] to-[#18222D] rounded-xl w-full max-w-3xl overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto border border-gray-700">
            <div className="flex justify-between items-center p-5 border-b border-gray-700 bg-[#1c2634]">
              ...
              <button onClick={() => setShowPaymentModal(false)} className="text-gray-400 hover:text-white transition-colors hover:bg-gray-700 rounded-full p-1">
                <X onClick={() => (window.location.href = "/endeavour/profile?events")} size={24} />
              </button>
            </div>
            <form onSubmit={handlePaymentSubmit} className="p-6 md:p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-gray-300 text-sm mb-2">Amount to Pay:</p>
                  <p className="text-[#00fcb8] font-bold text-2xl">₹{discountedPrice}</p>

                  <div className="mt-4">
                    <label htmlFor="paymentTransactionId" className="block text-sm font-medium text-gray-300 mb-2">Transaction ID</label>
                    <input id="paymentTransactionId" name="paymentTransactionId" type="text" value={formData.paymentTransactionId} onChange={handleChange} required className="bg-[#18222D] block w-full px-4 py-3 border border-gray-700 rounded-md text-gray-300 sm:text-sm" placeholder="Enter transaction ID" />
                  </div>

                  <div className="mt-4">
                    <label htmlFor="paymentScreenshot" className="block text-sm font-medium text-gray-300 mb-2">Payment Screenshot</label>
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-700 border-dashed rounded-lg cursor-pointer bg-[#18222D] hover:bg-[#111920]">
                      <div className="pt-5 pb-6 text-gray-400 text-center">
                        ...
                        <p className="text-xs mt-1 text-gray-500">{formData.paymentScreenshot?.name || "PNG, JPG (MAX. 5MB)"}</p>
                      </div>
                      <input id="paymentScreenshot" name="paymentScreenshot" type="file" accept="image/*" className="hidden" onChange={handleFileChange} required />
                    </label>
                  </div>
                </div>
                <div className="flex flex-col items-center justify-center bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="bg-white p-3 rounded-lg shadow-md">
                    <img src={qrCode} alt="Payment QR Code" className="w-full max-w-xs object-contain" />
                  </div>
                </div>
              </div>
